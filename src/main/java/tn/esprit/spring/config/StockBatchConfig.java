...
import tn.esprit.spring.batch.StockProcessor;
import tn.esprit.spring.batch.StockWriter;
...
public class StockBatchConfig {
	 ...
	 * fichier excel*/
	private String names = "libelleStock,qteStock,qteMin";
/*6. Définir la stratégie de délimitation des différents champs*/
	...
	public Step stockStep() {
		return stepBuilderFactory.get(STEP_NAME).<Stock, Stock>chunk(5).reader(stockItemReader())
	...
	public LineMapper<Stock> stockLineMapper() {
		...
		defaultLineMapper.setFieldSetMapper(fieldSet -> {
			...
			data.setQteStock(fieldSet.readInt(1));
